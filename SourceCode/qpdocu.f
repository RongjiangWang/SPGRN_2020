...
c
      do ig=1,ngrn
        ly=min0(lyr,lygrn(ig))
        depthrange=rrup(ly)
        rsdep=dabs(rrup(lyr)-rrup(lygrn(ig)))
        call tpssub(1+ly0-ly,hpmod(ly),vpmod(ly),rsdep,depthrange,
     &              head,diff,nr,dism,tp(1,ig),tkftp(1,ig),slwtp(1,ig))
        if(dmin1(vsmod(lyr),vsmod(lygrn(ig))).le.0.d0)then
          do i=1,nr
            ts(i,ig)=tp(i,ig)+dble(nt-1)*dt
            tkfts(i,ig)=tkftp(i,ig)
            slwts(i,ig)=2.d0*slwtp(i,ig)
          enddo
        else
          call tpssub(1+ly0-ly,hpmod(ly),vsmod(ly),rsdep,depthrange,
     &              head,diff,nr,dism,ts(1,ig),tkfts(1,ig),slwts(1,ig))
        endif
      enddo
